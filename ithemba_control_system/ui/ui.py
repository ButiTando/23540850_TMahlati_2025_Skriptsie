...
button_state = {
    ...
    # Lens field 11: the degrader ran the command but this lens did not move.
    "unchanged": {"value": 5, "text": "Unchanged", "color": "gray"},
}

# Mirrors the 2-bit lens field in the response word
# (design.tex: 00 OFF, 01 ON, 10 UPDATING, 11 NOT CHANGED).
lens_state = {
    0: "off",
    1: "on",
    2: "updating",
...
class MainWidget:
    def __init__(self, root):
        ...
        self.lens_btn_state = {"2mm": {"current":"off", "previous": "off", "desired_state": 0}, "3mm":  {"current":"off", "previous": "off", "desired_state": 0},
                               "6mm":  {"current":"off", "previous": "off", "desired_state": 0}, "8mm":  {"current":"off", "previous": "off", "desired_state": 0},
                               "10mm":  {"current":"off", "previous": "off", "desired_state": 0},"12mm":  {"current":"off", "previous": "off", "desired_state": 0},
    ...
    def add_camera_widget(self):
        ...
        camera_ip_addr_frame_btn = tk.Button(camera_ip_addr_frame,
                                      text="Connect",
                                      command= lambda sys_id=System_id.Camera_id.value, conn_item=connection_label: self.call_start_sytem(sys_id, connection_label))
    ...
    def add_degrader_control_widget(self):
        ...
        ip_addr_frame_btn = tk.Button(ip_addr_frame,
                                    text="Connect",
                                    command=lambda sys_id=System_id.Degrader_id.value, conn_label=connection_label: self.call_start_sytem(sys_id, conn_label))
    ...
    def add_xy_platform_control_widget(self):
        ...
        xy_platform_ip_frame_btn = tk.Button(xy_platform_ip_addr_frame,
                                      text="Connect",
                                      command= lambda sys_id=System_id.XY_Platform_id.value: self.call_start_sytem(sys_id))
        ...
        horizontal_feedback_label = tk.Label(feedback_frame, text="Horizonal position(mm)", font=("Arial", 10, "bold"))
    ...
    def add_dlm_control_widget(self):
        ...
        # make ip field available to oject
        ...
        blm_ip_frame_btn = tk.Button(blm_ip_addr_frame,
                                      text="Connect",
                                      command= lambda sys_id=System_id.BLM_id.value: self.call_start_sytem(sys_id, connection_label))
    ...
    def open_info_window(self, index):
        ...
        if system_id == System_id.Camera_id:
            ...
            # === Create Test_Camera instance ===
            cam = Test_Camera(view_frame, None)  # IP will be set on connect
            cam.connected_widget = status_label

            # === Connect function ===
            def connect_camera():
                ip = entry.get().strip()
                if not ip:
                    status_label.config(text="Invalid IP", fg="red")
                    return
                ...
                # Update camera
                cam.camera_ip = ip
                cam.rtsp_url = f'http://{ip}/snap.jpeg'
    ...
    def call_start_sytem(self, sys_id, connection_label):
        if(sys_id == System_id.BLM_id.value):
            ...
            started = self.sub_system_blm.start_server()

            # Update UI to reflect what actually happened, so a port that could
            # not be bound no longer shows a green "Connected".
            self.blm_connection_label = connection_label
            if connection_label:
                if started:
                    connection_label.config(text="Connected", fg="green")
                else:
                    connection_label.config(text="Port in use", fg="red")
            ...
            self.cam.caputure()
            print(f'Connecting to:{sys_id} with ip address:{camera_ip_addr}')
            pass
    ...
    def updated_degrader_button_states(self):
        try:
            ...
            with self.sub_system_degrader.response_lock:
                    current_lense_state :Response = self.sub_system_degrader.resp
            if(current_lense_state.process_status == 1 or current_lense_state.process_status == 3):
                self.updating_degrader = False
            

            if(self.updating_degrader):                
                
                self.lens_btn_state["2mm"]["current"] = lens_state[current_lense_state.lens_status_2mm]
                self.lens_btn_state["3mm"]["current"] = lens_state[current_lense_state.lens_status_3mm]
                self.lens_btn_state["6mm"]["current"] = lens_state[current_lense_state.lens_status_6mm]
                self.lens_btn_state["8mm"]["current"] = lens_state[current_lense_state.lens_status_8mm]
                self.lens_btn_state["10mm"]["current"] = lens_state[current_lense_state.lens_status_10mm]
                self.lens_btn_state["12mm"]["current"] = lens_state[current_lense_state.lens_status_12mm]
                self.lens_btn_state["30mm"]["current"] = lens_state[current_lense_state.lens_status_30mm]
    ...
    def _create_degrader_popup(self, parent):
        ...
        # Update button
        tk.Button(content, text="Update", bg="lightgray",
    ...
    def _connect_blm_popup(self, entry):
        ip = entry.get().strip() or DOSIMETER_DEFAULT_IP
        self.blm_ip_addr_frame_input.delete(0, tk.END)
        self.blm_ip_addr_frame_input.insert(0, ip)
        self.call_start_sytem(System_id.BLM_id.value, None)
