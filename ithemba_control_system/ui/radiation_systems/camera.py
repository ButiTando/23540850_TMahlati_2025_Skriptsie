...
class Test_Camera:
    ...
    def __init__(self, parent,camera_ip):
        self.rtsp_url = f'http://{camera_ip}/snap.jpeg'
    ...
    def caputure(self):
        ...
        if (Test_mode == False):
            try:
                ...
                if not self.cap.isOpened():
                    ...
                    # Start periodic caputer of frame.
                    self.update_frame()
        ...
        if not self.cap.isOpened():
            ...
            return print(f'{self.CAM_TAG["stream_load_error"]}: Failed to connect to {self.rtsp_url}')
        ...
        while self.running:
            try:
                if(Test_mode == False):
                    ...
                    response = requests.get(self.rtsp_url, timeout=5)
